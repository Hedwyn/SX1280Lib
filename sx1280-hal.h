...
public:
    ...
    /*!
     * \brief Writes a single bytes on the radio UART serial port
     *
     */
    void UartPutc(const uint8_t &c);


    /*!
     * \brief Reads a single bytes from the radio UART serial port
     *
     */
...
protected:
    ...
    BufferedSerial *RadioUart;                              //!< The UART object used to communicate with the radio
     ...
     */
    virtual void IoIrqInit( Callback<void()>  irqHandler );
};
